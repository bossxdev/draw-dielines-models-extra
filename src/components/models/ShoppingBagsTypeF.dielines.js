...
const MainDiecuts = (options) => {
  const { length, width, height, glueFlap, glueFlapSlope, colors, E, bottomGlue, handHoldingLength, handHoldingWidth, handHoldingMargin, coverFlap, topTuckFlap, lockMargin, labelUnits } = options;
  ...
  const handHoldingLine1 = []
  handHoldingLine1.push(new THREE.Vector2(((length / 2) - ((handHoldingLength / 2) - ((handHoldingWidth / 4) * 3))), (height - handHoldingMargin)))
  handHoldingLine1.push(new THREE.Vector2(((length / 2) + ((handHoldingLength / 2) - ((handHoldingWidth / 4) * 3))), (height - handHoldingMargin)))
  const handHoldingLine1Geometry = new THREE.Line(new THREE.BufferGeometry().setFromPoints(handHoldingLine1), colors.cr_red)
  handHoldingLine1Geometry.computeLineDistances()

  const handHoldingLine2 = []
  handHoldingLine2.push(new THREE.Vector2(((length / 2) - ((handHoldingLength / 2) - ((handHoldingWidth / 4) * 3))), (height - (handHoldingMargin + handHoldingWidth))))
  handHoldingLine2.push(new THREE.Vector2(((length / 2) + ((handHoldingLength / 2) - ((handHoldingWidth / 4) * 3))), (height - (handHoldingMargin + handHoldingWidth))))
  const handHoldingLine2Geometry = new THREE.Line(new THREE.BufferGeometry().setFromPoints(handHoldingLine2), colors.cr_red)
  handHoldingLine2Geometry.computeLineDistances()

  const curveLeftPoint = []
  for (let i = 0; i <= 180; i++) {
    curveLeftPoint.push(new THREE.Vector2( Math.sin(i * -(Math.PI / 180)) * (handHoldingWidth / 2), Math.cos(i * -(Math.PI / 180)) * (handHoldingWidth / 2)))
  }

  const curveLeftShape = new THREE.Shape()
  curveLeftShape.holes.push(new THREE.Path().setFromPoints(curveLeftPoint))
  const curveLeftLine = new THREE.Line(new THREE.ShapeGeometry(curveLeftShape), colors.cr_red)

  const handHoldingCurveLeft = new THREE.Object3D()
  handHoldingCurveLeft.position.set(((length / 2) - ((handHoldingLength / 2) - ((handHoldingWidth / 4) * 3))), height - (handHoldingMargin + (handHoldingWidth / 2)) ,0)
  handHoldingCurveLeft.add(curveLeftLine)

  const curveRightPoint = []
  for (let i = 0; i <= 180; i++) {
    curveRightPoint.push(new THREE.Vector2( Math.sin(i * (Math.PI / 180)) * (handHoldingWidth / 2), Math.cos(i * (Math.PI / 180)) * (handHoldingWidth / 2)))
  }

  const curveRightShape = new THREE.Shape()
  curveRightShape.holes.push(new THREE.Path().setFromPoints(curveRightPoint))
  const curveRightLine = new THREE.Line(new THREE.ShapeGeometry(curveRightShape), colors.cr_red)

  const handHoldingCurveRight = new THREE.Object3D()
  handHoldingCurveRight.position.set(((length / 2) + ((handHoldingLength / 2) - ((handHoldingWidth / 4) * 3))), height - (handHoldingMargin + (handHoldingWidth / 2)) ,0)
  handHoldingCurveRight.add(curveRightLine)
  
  const handHoldingShape = new THREE.Object3D()
  handHoldingShape.add(handHoldingCurveLeft, handHoldingCurveRight, handHoldingLine1Geometry, handHoldingLine2Geometry)

  const coverLockPoint1 = []
  coverLockPoint1.push(new THREE.Vector2((glueFlap + (length / 2)) - (topTuckFlap / 2), ((E + height)  - (coverFlap - lockMargin))))
  coverLockPoint1.push(new THREE.Vector2( (glueFlap + (length / 2)) + (topTuckFlap / 2), ((E + height)  - (coverFlap - lockMargin))))
  const coverLock1GeometryLine = new THREE.Line(new THREE.BufferGeometry().setFromPoints(coverLockPoint1), colors.cr_red)
  coverLock1GeometryLine.computeLineDistances()

  const coverLockPoint2 = []
  coverLockPoint2.push(new THREE.Vector2((glueFlap + (length / 2)) - (topTuckFlap / 2), ((E + height)  - ((coverFlap - lockMargin) - lockWidth))))
  coverLockPoint2.push(new THREE.Vector2( (glueFlap + (length / 2)) + (topTuckFlap / 2), ((E + height)  - ((coverFlap - lockMargin) - lockWidth))))
  const coverLock2GeometryLine = new THREE.Line(new THREE.BufferGeometry().setFromPoints(coverLockPoint2), colors.cr_red)
  coverLock2GeometryLine.computeLineDistances()

  const coverLockCurveLeftPoint = []
  for (let i = 0; i <= 180; i++) {
    coverLockCurveLeftPoint.push(new THREE.Vector2( Math.sin(i * -(Math.PI / 180)) * (lockWidth / 2), Math.cos(i * -(Math.PI / 180)) * (lockWidth / 2)))
  }

  const coverLockCurveLeftShape = new THREE.Shape()
  coverLockCurveLeftShape.holes.push(new THREE.Path().setFromPoints(coverLockCurveLeftPoint))
  const coverLockCurveLeftLine = new THREE.Line(new THREE.ShapeGeometry(coverLockCurveLeftShape), colors.cr_red)

  const coverLockCurveLeft = new THREE.Object3D()
  coverLockCurveLeft.position.set((glueFlap + (length / 2)) - (topTuckFlap / 2), ((E + height)  - ((coverFlap - lockMargin) - (lockWidth / 2))), 0)
  coverLockCurveLeft.add(coverLockCurveLeftLine)

  const coverLockCurveRightPoint = []
  for (let i = 0; i <= 180; i++) {
    coverLockCurveRightPoint.push(new THREE.Vector2( Math.sin(i * (Math.PI / 180)) * (lockWidth / 2), Math.cos(i * (Math.PI / 180)) * (lockWidth / 2)))
  }

  const coverLockCurveRightShape = new THREE.Shape()
  coverLockCurveRightShape.holes.push(new THREE.Path().setFromPoints(coverLockCurveRightPoint))
  const coverLockCurveRightLine = new THREE.Line(new THREE.ShapeGeometry(coverLockCurveRightShape), colors.cr_red)

  const coverLockCurveRight = new THREE.Object3D()
  coverLockCurveRight.position.set((glueFlap + (length / 2)) + (topTuckFlap / 2), ((E + height)  - ((coverFlap - lockMargin) - (lockWidth / 2))), 0)
  coverLockCurveRight.add(coverLockCurveRightLine)
  ...
  const coverLockPoint = []
  for (let i = 90; i <= 270; i++) {
    coverLockPoint.push(new THREE.Vector2( Math.sin(i * (Math.PI / 180)) * (topTuckFlap / 2), Math.cos(i * (Math.PI / 180)) * (topTuckFlap / 2)))
  }

  const coverLockShape = new THREE.Shape()
  coverLockShape.holes.push(new THREE.Path().setFromPoints(coverLockPoint))
  const coverLockLine = new THREE.Line(new THREE.ShapeGeometry(coverLockShape), colors.cr_red)

  const coverLockLine1 = []
  coverLockLine1.push(new THREE.Vector2(-((topTuckFlap / 2) + 3), 0))
  coverLockLine1.push(new THREE.Vector2(-(topTuckFlap / 2), 0))
  const coverLock1Geometry = new THREE.Line(new THREE.BufferGeometry().setFromPoints(coverLockLine1), colors.cr_red)
  coverLock1Geometry.computeLineDistances()

  const coverLockLine2 = []
  coverLockLine2.push(new THREE.Vector2((topTuckFlap / 2), 0))
  coverLockLine2.push(new THREE.Vector2((topTuckFlap / 2) + 3, 0))
  const coverLock2Geometry = new THREE.Line(new THREE.BufferGeometry().setFromPoints(coverLockLine2), colors.cr_red)
  coverLock2Geometry.computeLineDistances()

  const coverLock = new THREE.Object3D()
  coverLock.position.set(length / 2, coverFlap - lockMargin, 0)
  coverLock.add(coverLockLine, coverLock1Geometry, coverLock2Geometry)

  const coverHandHolding = new THREE.Object3D()
  coverHandHolding.position.set(0, -(height - ((handHoldingMargin * 2) + handHoldingWidth)), 0)
  coverHandHolding.add(handHoldingShape.clone())


  const Group_A_Top = new THREE.Object3D()
  Group_A_Top.position.set(glueFlap, E, 0)
  Group_A_Top.add(A1_Geometry, A2_Geometry, handHoldingShape)
  ...
  GroupCoverTop.add(coverTop1GeometryLine, coverTop2GeometryLine, coverTop3GeometryLine, coverTopCurveLeft, coverTopCurveRight, coverHandHolding, coverLock)
  ...
  PlaneGroupClone.add(Group_A_Top.clone(), Group_B_Top.clone(), Group_B_Top2.clone(), Group_E.clone(), coverLock1GeometryLine, coverLock2GeometryLine, coverLockCurveLeft, coverLockCurveRight)
...
const MainLays_Half_B = (options) => {
  ...
  PlaneGroupClone.add(Group_A_Top, Group_B_Top.clone(), Group_B_Top2.clone(), Group_E.clone(), coverLock1GeometryLine, coverLock2GeometryLine, coverLockCurveLeft, coverLockCurveRight)
