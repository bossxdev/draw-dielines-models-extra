...
const MainDiecuts = (options) => {
  ...
  sideB1.position.set(0, set_bottom, 0)
  sideB1.add(side_b1)

  const sideB2 = new THREE.Object3D()
  sideB2.position.set(width + length, set_bottom, 0)
  sideB2.add(side_b2)

  const sideA1 = new THREE.Object3D()
  sideA1.position.set(width, set_bottom, 0)
  sideA1.add(side_a1)

  const sideA2 = new THREE.Object3D()
  sideA2.position.set((width * 2) + length, set_bottom, 0)
  sideA2.add(side_a2)

  const cover_top = new THREE.Object3D()
  cover_top.position.set((width * 2) + length, set_bottom + height, 0)
  cover_top.add(parts.boxCoverTypeA())

  const coverBottomLeft = new THREE.Object3D()
  coverBottomLeft.position.set(width, set_bottom, 0)
  coverBottomLeft.add(parts.boxCoverTypeC())

  const coverBottomRight = new THREE.Object3D()
  coverBottomRight.position.set((width * 2) + length, set_bottom, 0)
  coverBottomRight.add(parts.boxCoverTypeD())

  const dustFlapBottomLeft = new THREE.Object3D()
  dustFlapBottomLeft.position.set(width, set_bottom, 0)
  dustFlapBottomLeft.rotation.y = Math.PI
  dustFlapBottomLeft.add(parts.dustFlapBottomTypeA())

  const dustFlapBottomRight = new THREE.Object3D()
  dustFlapBottomRight.position.set(width + length, set_bottom, 0)
  dustFlapBottomRight.add(parts.dustFlapBottomTypeA().clone())

  const dustFlapRight = new THREE.Object3D()
  dustFlapRight.position.set(length + width, set_bottom + height, 0)
  dustFlapRight.add(parts.dustFlapTypeA())

  const dustFlapLeft = new THREE.Object3D()
  dustFlapLeft.rotation.y = Math.PI
  dustFlapLeft.position.set(width, set_bottom + height, 0)
  dustFlapLeft.add(parts.dustFlapTypeA())

  const glue_lid = new THREE.Object3D()
  glue_lid.rotation.y = Math.PI
  glue_lid.position.set((width * 2) + (length * 2) + glueFlap, set_bottom, 0)
  ...
  lineMarkA.position.set(width + (length / 2), set_bottom + (height / 1.5), 2)
  lineMarkA.add(lengthImg)

  const lineMarkA2 = new THREE.Object3D()
  lineMarkA2.position.set((width * 2) + length + (length / 2), set_bottom + (height / 1.5), 2)
  lineMarkA2.add(lengthImg.clone())

  const lineMarkB = new THREE.Object3D()
  lineMarkB.position.set( (width / 2), set_bottom + (height / 1.5), 2)
  lineMarkB.add(widthImg)

  const lineMarkB2 = new THREE.Object3D()
  lineMarkB2.position.set(width + length + (width / 2), set_bottom + (height / 1.5), 2)
  ...
  lineMarkF.position.set(width + length + (width / 4), set_bottom + height + (dustFlap / 2), 2)
  lineMarkF.add(dustFlapImg)

  const lineMarkP = new THREE.Object3D()
  lineMarkP.position.set((width * 2) + length + (length / 3), set_bottom + height + width + (topFlap / 2), 2)
  lineMarkP.add(topFlapImg)

  const lineMarkG = new THREE.Object3D()
  lineMarkG.position.set((width * 2) + (length * 2) + (glueFlap / 2), set_bottom + (height / 1.5), 2)
  lineMarkG.add(glueFlapImg)

  const labelLength = new THREE.Object3D()
  labelLength.position.set(width + (length / 2), set_bottom + (height / 1.8), 2)

  const labelLength1 = new THREE.Object3D()
  labelLength1.position.set((width * 2) + length + (length / 2), set_bottom + (height / 1.8), 2)

  const labelWidth = new THREE.Object3D()
  labelWidth.position.set((width / 2), set_bottom + (height / 1.8), 2)

  const labelWidth2 = new THREE.Object3D()
  labelWidth2.position.set(width + length + (width / 2), set_bottom + (height / 1.8), 2)

  const labelWidth3 = new THREE.Object3D()
  labelWidth3.rotation.z = Math.PI / 2
  labelWidth3.position.set((width * 2) + length + (length / 2) + 7, width / 2 + height + set_bottom, 2)
  ...
  labelGlueFlap.position.set((width * 2) + (length * 2) + (glueFlap / 2), set_bottom + (height / 1.8), 2)

  const labelTopFlap = new THREE.Object3D()
  labelTopFlap.rotation.z = Math.PI / 2
  labelTopFlap.position.set((width * 2) + length + (length / 2) + 7, set_bottom + height + width + (topFlap / 2), 2)

  const labelDustFlap = new THREE.Object3D()
  labelDustFlap.rotation.z = Math.PI / 2
  labelDustFlap.position.set(width + length + (width / 2) + 7, set_bottom + height + (dustFlap / 2), 2)
  ...
  topFlapArrow.position.set((width * 2) + length + (length / 2), set_bottom + height + width + topFlap, 2)
  topFlapArrow.add( arrows(topFlap).clone())

  const dustFlapArrow = new THREE.Object3D()
  dustFlapArrow.rotation.z = -Math.PI / 2
  dustFlapArrow.position.set(width + length + (width / 2), set_bottom + height + dustFlap, 1)
  dustFlapArrow.add( arrows(dustFlap).clone())

  const glueFlapArrow = new THREE.Object3D()
  glueFlapArrow.position.set((width * 2) + (length * 2), set_bottom + (height / 2), 1)
...
const MainLays = (options) => {
  ...
  const dustFlapRight = new THREE.Object3D()
  dustFlapRight.position.set(width + length, set_bottom + height, 0)
  dustFlapRight.add(parts.dustFlapTypeA(), parts.dustFlapGeometryTypeA())

  const dustFlapLeft = new THREE.Object3D()
  dustFlapLeft.rotation.y = Math.PI
  dustFlapLeft.position.set(width, set_bottom + height, 0)
  dustFlapLeft.add(parts.dustFlapTypeA(), parts.dustFlapGeometryTypeAInvert())

  const dustFlapBottomRight = new THREE.Object3D()
  dustFlapBottomRight.position.set(width + length, set_bottom, 0)
  dustFlapBottomRight.add(parts.dustFlapBottomTypeA(), parts.dustFlapBottomGeometryTypeA2())
  
  const dustFlapBottomLeft = new THREE.Object3D()
  dustFlapBottomLeft.rotation.y = Math.PI
  dustFlapBottomLeft.position.set(width, set_bottom, 0)
  dustFlapBottomLeft.add(parts.dustFlapBottomTypeA().clone(), parts.dustFlapBottomGeometryTypeAInvert2())

  const glue_lid = new THREE.Object3D()
  glue_lid.rotation.y = Math.PI
  glue_lid.rotation.x = Math.PI
  glue_lid.position.set((width * 2) + (length * 2) + glueFlap, set_bottom + height, 0)
...
const MainLays_Invert = (options) => {
  ...
  const sideB1 = new THREE.Object3D()
  sideB1.rotation.y = Math.PI
  sideB1.position.set(glueFlap + length + width, set_bottom, 0)
  sideB1.add(side_b2, parts.planeSideBGeometryInvert())

  const sideB2 = new THREE.Object3D()
  sideB2.rotation.y = Math.PI
  sideB2.position.set(glueFlap + length + (width * 2) + length, set_bottom, 0)
  sideB2.add(side_b1, parts.planeSideBGeometryInvert())

  const sideA1 = new THREE.Object3D()
  sideA1.rotation.y = Math.PI
  sideA1.position.set(glueFlap + length, set_bottom, 0)
  sideA1.add(side_a2, parts.planeSideAGeometryInvert())

  const sideA2 = new THREE.Object3D()
  sideA2.rotation.y = Math.PI
  sideA2.position.set(glueFlap + width + (length * 2), set_bottom, 0)
  sideA2.add(side_a1, parts.planeSideAGeometryInvert())

  const cover_top = new THREE.Object3D()
  cover_top.rotation.y = Math.PI
  cover_top.position.set(glueFlap + length, set_bottom + height, 0)
  cover_top.add(parts.boxCoverTypeA(), parts.boxCoverGeometryTypeAInvert())

  const coverBottomLeft = new THREE.Object3D()
  coverBottomLeft.rotation.y = Math.PI
  coverBottomLeft.position.set(glueFlap + length, set_bottom, 0)
  coverBottomLeft.add(parts.boxCoverTypeD(), parts.boxCoverGeometryTypeDInvert())

  const coverBottomRight = new THREE.Object3D()
  coverBottomRight.rotation.y = Math.PI
  coverBottomRight.position.set(glueFlap + (length * 2) + width, set_bottom, 0)
  coverBottomRight.add(parts.boxCoverTypeC(), parts.boxCoverGeometryTypeCInvert())

  const dustFlapLeft = new THREE.Object3D()
  dustFlapLeft.position.set(glueFlap + width + (length * 2) , set_bottom + height, 0)
  dustFlapLeft.add(parts.dustFlapTypeA(), parts.dustFlapGeometryTypeA())

  const dustFlapRight = new THREE.Object3D()
  dustFlapRight.rotation.y = Math.PI
  dustFlapRight.position.set(glueFlap + length + width, set_bottom + height, 0)
  dustFlapRight.add(parts.dustFlapTypeA(), parts.dustFlapGeometryTypeAInvert())

  const dustFlapBottomRight = new THREE.Object3D()
  dustFlapBottomRight.rotation.y = Math.PI
  dustFlapBottomRight.position.set(glueFlap + length + width, set_bottom, 0)
  dustFlapBottomRight.add(parts.dustFlapBottomTypeA(), parts.dustFlapBottomGeometryTypeAInvert2())

  const dustFlapBottomLeft = new THREE.Object3D()
  dustFlapBottomLeft.position.set(glueFlap + (length * 2) + width, set_bottom, 0)
  dustFlapBottomLeft.add(parts.dustFlapBottomTypeA().clone(), parts.dustFlapBottomGeometryTypeA2())

  const glue_lid = new THREE.Object3D()
  glue_lid.rotation.x = Math.PI
  glue_lid.position.set(0, set_bottom + height, 0)
