...
const MainDiecuts = (options) => {
  const { length, width, height, glueFlap, colors, E, bottomGlue, handHoldingLength, handHoldingWidth, handHoldingMargin, labelUnits } = options;
  ...
  // G1.push(new THREE.Vector2(0, E + height))
  ...
  redLine.push(new THREE.Vector2(0, height + E))
  ...
  heightDimensionLabel.position.set(-glueFlap, E / 2 + height / 2, 2)

  const widthDimensionLabel = new THREE.Object3D()
  widthDimensionLabel.position.set(glueFlap + length + width, E + height + 20, 2)
  ...
  loaderTextHeightDimension.load('./fonts/Mitr Medium_Regular.typeface.json', function (font) {
    ...
    const message = `${labelUnits === 'inch' ? ((E + height) / defaultUnit[labelUnits]).toFixed(2) : ((E + height) / defaultUnit[labelUnits]).toFixed(1)} ${labelUnits}`
    const shapes = font.generateShapes(message, 7)
    const geometry = new THREE.ShapeBufferGeometry(shapes)
    geometry.computeBoundingBox()
  ...
  heightDimensionArrow.position.set(-(glueFlap / 2), E + height, 2)
  heightDimensionArrow.add(
      arrows(E + height).clone(),
      lineSet(0, 0, 0, 10).clone(),
      lineSet(E + height, 0, E + height, 10).clone()
  )

  const widthDimensionArrow = new THREE.Object3D()
  widthDimensionArrow.position.set(0, E + height + 10, 2)
  ...
  PositionCenter.position.set(-(glueFlap + length * 2 + width * 2) / 2, -((E + height) / 2), 0)
