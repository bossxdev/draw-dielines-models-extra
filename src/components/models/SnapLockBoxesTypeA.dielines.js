...
const MainDiecuts = (options) => {
  ...
  const coverTop = new THREE.Object3D()
  coverTop.position.set(glueFlap, set_bottom + height, 0)
  coverTop.add(parts.boxCoverTypeA())
  ...
  dustFlapLeft.position.set(glueFlap + width + length, set_bottom + height, 0)
  dustFlapLeft.add(parts.dustFlapTypeA())

  const dustFlapRight = new THREE.Object3D()
  dustFlapRight.position.set(glueFlap + width + length * 2, set_bottom + height, 0)
  ...
  lineMarkF.position.set(glueFlap + length + (width / 3), set_bottom + height + (dustFlap / 2), 2)
  lineMarkF.add(dustFlapImg)

  const lineMarkP = new THREE.Object3D()
  lineMarkP.position.set(glueFlap + (length / 2.5), set_bottom + height + width + (topFlap / 2), 2)
  ...
  labelWidth3.position.set(glueFlap + (length / 2) + 5, width / 2 + height + set_bottom, 2)
  ...
  labelTopFlap.position.set(glueFlap + (length / 2) + 7, set_bottom + height + width + (topFlap / 2), 2)

  const labelDustFlap = new THREE.Object3D()
  labelDustFlap.rotation.z = Math.PI / 2
  labelDustFlap.position.set(glueFlap + length + (width / 2) + 7, set_bottom + height + (dustFlap / 2), 2)
  ...
  widthArrow3.position.set(glueFlap + (length / 2), set_bottom + height + width, 2)
  ...
  topFlapArrow.position.set(glueFlap + (length / 2), set_bottom + height + width + topFlap, 2)
  topFlapArrow.add( arrows(topFlap).clone())

  const dustFlapArrow = new THREE.Object3D()
  dustFlapArrow.rotation.z = -Math.PI / 2
  dustFlapArrow.position.set(glueFlap + length + (width / 2), set_bottom + height + dustFlap, 1)
...
const MainLays = (options) => {
  ...
  const coverTop = new THREE.Object3D()
  coverTop.position.set(glueFlap, set_bottom + height, 0)
  coverTop.add(parts.boxCoverTypeA(), parts.boxCoverGeometryTypeA())
  ...
  dustFlapBottomLeft.add(parts.dustFlapBottomTypeA(), parts.dustFlapBottomGeometryTypeAInvert1())

  const dustFlapBottomRight = new THREE.Object3D()
  dustFlapBottomRight.position.set(glueFlap + (length * 2) + (width * 2), set_bottom, 0)
  dustFlapBottomRight.rotation.y = Math.PI
  dustFlapBottomRight.add(parts.dustFlapBottomTypeA(), parts.dustFlapBottomGeometryTypeA1())
  ...
  const grid_all = new THREE.Object3D()
  grid_all.position.set(0, 0, 0)
  // grid_all.position.set(-(glueFlap + length * 2 + width * 2) / 2, -(height + set_bottom + width + topFlap) / 2, 0)
  scene.add(grid_all)
  grid_all.add(
    sideA1,
    sideA2,
...
const MainLays_Invert = (options) => {
  ...
  const coverTop = new THREE.Object3D()
  coverTop.position.set(glueFlap, set_bottom + height, 0)
  coverTop.add(parts.boxCoverTypeA(), parts.boxCoverGeometryTypeAInvert())
  ...
  const dustFlapBottomLeft = new THREE.Object3D()
  dustFlapBottomLeft.position.set(glueFlap + length + width + length, set_bottom, 0)
  dustFlapBottomLeft.add(parts.dustFlapBottomTypeA(), parts.dustFlapBottomGeometryTypeAInvert2())

  const dustFlapBottomRight = new THREE.Object3D()
  dustFlapBottomRight.position.set(glueFlap + length + width, set_bottom, 0)
  dustFlapBottomRight.rotation.y = Math.PI
  dustFlapBottomRight.add(parts.dustFlapBottomTypeA().clone(), parts.dustFlapBottomGeometryTypeA2())
