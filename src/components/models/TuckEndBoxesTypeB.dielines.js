...
const MainDiecuts = (options) => {
  ...
  cover_top.position.set(glueFlap + length + width, height, 0)
  ...
  flap_right.position.set(glueFlap + length + width + length + width, height, 0)
  ...
  flap_left.position.set(glueFlap + length, height, 0)
  ...
  lineMarkF.position.set(glueFlap + (length * 2) + width + (width / 3),  height + (dustFlap / 2), 2)
  lineMarkF.add(dustFlapImg)

  const lineMarkP = new THREE.Object3D()
  lineMarkP.position.set(glueFlap + length + width + (length / 2.5),  height + width + (topFlap / 2), 2)
  ...
  labelWidth3.position.set(glueFlap + length + width + (length / 2) + 5, width / 2 + height , 2)
  ...
  labelTopFlap.position.set(glueFlap + length + width + (length / 2) + 7,  height + width + (topFlap / 2), 2)

  const labelDustFlap = new THREE.Object3D()
  labelDustFlap.rotation.z = Math.PI / 2
  labelDustFlap.position.set(glueFlap + (length * 2) + width + (width / 2) + 7,  height + (dustFlap / 2), 2)
  ...
  widthArrow3.position.set(glueFlap + length + width + (length / 2),  height + width, 2)
  ...
  topFlapArrow.position.set(glueFlap + length + width + (length / 2),  height + width + topFlap, 2)
  topFlapArrow.add( arrows(topFlap).clone())

  const dustFlapArrow = new THREE.Object3D()
  dustFlapArrow.rotation.z = -Math.PI / 2
  dustFlapArrow.position.set(glueFlap + (length * 2) + width + (width / 2),  height + dustFlap, 1)
...
const MainLays = (options) => {
  ...
  const flap_left_d = new THREE.Object3D()
  flap_left_d.position.set(glueFlap + length, 0, 1)
  flap_left_d.rotation.x = Math.PI
  flap_left_d.add(parts.dustFlapTypeA(), parts.dustFlapGeometryTypeAInvert())

  const flap_right = new THREE.Object3D()
  flap_right.position.set(glueFlap + length + width + length + width, height, 1)
  flap_right.rotation.y = Math.PI
...
const MainLays_Invert = (options) => {
  ...
  const PositionCenter = new THREE.Object3D()
  // PositionCenter.position.set((glueFlap + length * 2 + width * 2) / 2, -((height + topFlap * 2 + width * 2) / 2), 0)
  PositionCenter.position.set(glueFlap + length * 2 + width * 2, 0, 1)
...
class main_box {
  constructor(width, depth, height, glue, glueFlapSlope, plug, topFlapSlope, flap, colors) {
    this.width = width
    this.depth = depth
    this.height = height
    this.glue = glue
    ...
    this.flap = flap
    this.color = colors
    this.plugs_slope = topFlapSlope
    this.glue_slope = glueFlapSlope
  }
